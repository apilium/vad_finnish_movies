...
MODELS = {
    ...
    'scratch': {
        ...
        'outputdim': 2,
        'encoder': 'labelencoders/students.pth',
        'pretrained': 'scratch/run_model_latest.pt',
        'resolution': 0.02
    }
}
...
Please download the pretrained models from and try again or set --pretrained_dir to your directory."""
    ...
    with torch.no_grad(), tqdm(total=len(dloader), leave=False,
        ...
        for feature, filename in dloader:
            ...
            print('printing tag shape:', prediction_tag.shape)
            print('printing time shape:', prediction_time.shape)
            ...
            if prediction_time is not None:  # Some models do not predict timestamps
                ...
                print ('printing speech soft pred shape: ', speech_soft_pred.shape)
                print ('printing speech soft pred length: ', len(speech_soft_pred) )
                 ####
                #import matplotlib.pyplot as plt
                #
                #speech_sayilar = speech_soft_pred.numpy()
                #plt.hist(np.arange(len(speech_soft_pred)), speech_sayilar)
                #plt.savefig('histogram_speech.png')
                #
                ###
