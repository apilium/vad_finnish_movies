...
class FrameBCELoss(nn.Module):
    ...
    def forward(self, clip_prob, frame_prob, tar_time, tar_clip, length):
        ...
        idxs = torch.arange(timesteps, device='cpu').repeat(batchsize).contiguous().view(
            batchsize, timesteps)
        mask = (idxs < length.contiguous().view(-1, 1)).to(frame_prob.device)
    ...
    def forward(self, clip_prob, frame_prob, tar_time, tar_clip, length):
        return self.frameloss(
            clip_prob, frame_prob, tar_time, tar_clip, length) + self.cliploss(
                clip_prob, tar_clip) ### frameloss can be removed if the teacher is performing well


class BCELossWithLabelSmoothing(nn.Module): ### could be an alternative loss
    """docstring for BCELoss"""
